...
// --- Nowy, płaski kształt oee.json z kolejnością: timestamp -> oee -> product -> internal -> helpers_* ---
...
type HelpersAir struct {
	...
	TotalCurrentL        float64            `json:"total_current_M3"`
...
func ResetOeeStateAndFile(path string) {
	...
	of = OeeFileFlat{
		...
		HelpersAir: HelpersAir{
			...
			TotalCurrentL:        fFrom(ha, "total_current_M3",        "helpers_air_total_current_M3"),
		...
		if CzasPomiarowy.PauseStartTime != nil {
			...
			if math.Abs(cycle-lastCycle) > 0.01 {
				...
				if dur <= config.MaxChangeoverDuration {
					...
					// 	"[OEE] Long changeover (%.0fs) counted as pause", dur,
					// ))
				}
			} else {
				// zwykła pauza
	...
	utils.Go("OEE WydajnoscTempUpdater", func() {
		...
		for range ticker.C {
			func() {
				...
				calcLock.Lock()
				curr := getOeeInt(data, "ilosc_elementow")
				delta := curr - lastElementCountOEE
				lastElementCountOEE = curr

				cyklLpm := utils.ToFloat(CalculatedData["cykl"])
				calcLock.Unlock()

				cykl := cyklLpm / 60.0
				expected := cykl * interval.Seconds()
...
func calculateWydajnosc() float64 {
	...
	var totalExpectedElements float64
	var totalActualElements float64

	for _, period := range cycleHistory {
		duration := period.EndTime.Sub(period.StartTime).Seconds()
		if period.CycleLPM > 0 {
			expected := duration / (60.0 / period.CycleLPM)
			totalExpectedElements += expected
			totalActualElements += float64(period.ElementCounter)
		}
	}

	currentDuration := now.Sub(currentCycleStartTime).Seconds()
	if currentCycleValue > 0 {
		expected := currentDuration / (60.0 / currentCycleValue)
		totalExpectedElements += expected
		totalActualElements += float64(currentCycleElementCnt)
	}

	if totalExpectedElements == 0 {
		return 0.0
	}
	return math.Round((totalActualElements/totalExpectedElements)*10000) / 10000
}
		...
		if arr, ok := in["cycle_history"].([]interface{}); ok {
			for _, it := range arr {
				if m, ok := it.(map[string]interface{}); ok {
					...
					cycleHistory = append(cycleHistory, CyclePeriod{
						StartTime: st, EndTime: en, CycleLPM: cyc, ElementCounter: cnt,
					})
				}
			}
		}
...
func saveOeeFlat(of OeeFileFlat, path string) error {
	...
	helpersAir := map[string]interface{}{
		...
		"total_current_M3":         of.HelpersAir.TotalCurrentL,
	}
